    ...
    public class JwtProvider(ILogger<JwtProvider> logger, IOptions<JwtOptions> options) : IJwtProvider
    {
        ...
        public Result<string> GenerateAccessToken(IEnumerable<Claim> claims)
        {
            try
            {
                ...
                var tokenDescriptor = GetTokenDescriptor(options.Secret, options.AccessTokenExpirationHours, options.AccessTokenExpirationMinutes, claims.ToArray());
        ...
        private static SecurityTokenDescriptor GetTokenDescriptor(string key, int expirationHours, int? expirationMinutes, params Claim[]? claims)
        {
            var keyBytes = Encoding.UTF8.GetBytes(key);
            var time = DateTime.UtcNow.AddHours(expirationHours);
            if (expirationMinutes.HasValue)
            {
                time = time.AddMinutes(expirationMinutes.Value);
            }
